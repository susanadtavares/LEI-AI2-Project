// File: FormandosPendentes.jsx
import { useEffect, useState, useCallback, useMemo } from "react";
import "./PedidosPendentes.css";
import api from "../../api";

export default function FormandosPendentes() {
  ...
  const fetchPedidos = useCallback(async () => {
    ...
    try {
      ...
      const formandos = res.data.filter((p) => p.tipo === "formando");
      console.log("Fetched pedidos structure:", formandos);
      if (formandos.length > 0 && !formandos[0].id_pedido) {
        console.warn("No 'id_pedido' found in pedidos data:", formandos[0]);
        setError("Os dados recebidos não contêm um ID válido (id_pedido). Contacte o administrador.");
      }
      setPedidos(formandos);
    } catch (err) {
      setError("Erro ao carregar os dados. Tente novamente.");
      console.error("Erro ao buscar formandos:", err.response?.data || err.message);
    } finally {
      setLoading(false);
    }
  }, []);

  const handleAction = useCallback(async (id_pedido, action) => {
    if (!id_pedido || isNaN(Number(id_pedido))) {
      alert("ID inválido para a ação. Verifique os dados no console ou contacte o administrador.");
      console.log(
    ...
    try {
      ...
      const response = await api.put(endpoint, {}, { headers: { "X-Request-Source": "web" } });
      console.log(`Action ${action} response:`, response.data);

      setPedidos((prevPedidos) =>
        prevPedidos.map((pedido) =>
          pedido.id_pedido === id_pedido
            ? { ...pedido, estado: action === "aprovar" ? "aprovado" : "rejeitado" }
            : pedido
        )
      );
      ...
      console.error(`Erro ao ${action} pedido:`, err.response?.data || err.message);
  ...
  const aprovar = useCallback((id_pedido) => handleAction(id_pedido, "aprovar"), [handleAction]);
  const rejeitar = useCallback((id_pedido) => {
    ...
    <div className="pagina-pendentes">
      ...
      {!loading && !error && (
        <div className="section">
          <h2><span className="heading-text">Pedidos de Formandos</span></h2>

          {pedidos.length === 0 ? (
            <p>Sem pedidos de formandos.</p>
          ) : (
            <>
              <div className="table-wrapper">
                <table role="grid" aria-label="Pedidos de Formandos">
                  <thead>
                    <tr>
                      {columns.map((col) => (
                  ...
                  <tbody>
                    {pageItems.map((item, index) => (
                      <tr key={item.id_pedido || index} className={index === pageItems.length - 1 ? "last-row" : ""}>
                        {columns.map((col) => (
