...
export default function EditarFormador() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get(`/formadores/${id_utilizador}/${id_formador}/full`);
        const d = res.data || {};
        const mapped = {
          nome: d.nome || '',
          email: d.email || '',
        ...
        alert('Erro ao carregar dados do formador.');
      }
    })();
  ...
  const validarCampo = (nome, valor) => {
    switch (nome) {
      ...
      case 'telemovel':
        if (!/^\d*$/.test(valor)) return 'O número de telemóvel só pode conter dígitos.';
        if (valor.length !== 9) return 'O número de telemóvel deve ter exatamente 9 dígitos.';
        if (!/^9[12356]\d{7}$/.test(valor)) return 'O número de telemóvel não é válido em Portugal.';
        return '';
  ...
  const handleChange = (e) => {
    ...
    setFormador(prev => ({ ...prev, [name]: name === 'data_nascimento' ? next : value }));
  ...
  const handleSubmit = async (e) => {
    ...
    setErroGeral('');
    setErros({});
    const { nome, email, telemovel, data_nascimento, genero } = formador;
    ...
    try {
      ...
      await api.put(`/formadores/${id_utilizador}/${id_formador}/full`, {
      ...
      setUploading(false);
      const msg = err.response?.data?.error || err.response?.data?.mensagem || err.message || 'Erro ao atualizar formador.';
      setErroGeral(msg);
    }
  };
  ...
  return (
    <div className="tf-page">
      <div className="formador-edit-ui">
        <h2><span className="tf-title">Editar Formador</span></h2>

        <form onSubmit={handleSubmit}>
          <div className="tf-form-grid">
            {/* Coluna da foto */}
            <aside className="tf-photo-col">
              <div className="tf-foto-preview">
                <div
                  className="tf-avatar-click"
                  onClick={() => fileInputRef.current?.click()}
                  title="Clique para escolher uma foto"
                >
                  {fotoPreview ? <img src={fotoPreview} alt="Foto de perfil" /> : <div className="tf-circle">Foto</div>}
                </div>

                <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFotoChange} hidden />

                <div className="tf-foto-actions">
                  <button type="button" className="tf-btn tf-btn-primary" onClick={() => fileInputRef.current?.click()}>
                ...
                {erroFoto && <span className="tf-msg-erro">{erroFoto}</span>}
                {uploading && <span style={{ fontSize: '.9rem' }}>A enviar imagem…</span>}
              </div>
            </aside>

            {/* Coluna dos campos */}
            <section className="tf-fields-col">
              <div className="tf-form-row tf-name-row">
                <div className="tf-form-group">
                  ...
                  <input
                    ...
                    required
                    className={erros.nome ? 'tf-erro-input' : ''}
                  />
                  {erros.nome && <span className="tf-msg-erro">{erros.nome}</span>}
                </div>
              </div>
              ...
              <div className="tf-form-row">
                <div className="tf-form-group">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    required
                    className={erros.email ? 'tf-erro-input' : ''}
                  />
                  {erros.email && <span className="tf-msg-erro">{erros.email}</span>}
                </div>
                <div className="tf-form-group">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    required
                    className={telemovelTocado && erros.telemovel ? 'tf-erro-input' : ''}
                    maxLength={9}
                    inputMode="numeric"
                  />
                  {telemovelTocado && erros.telemovel && <span className="tf-msg-erro">{erros.telemovel}</span>}
                </div>
              </div>
              ...
              <div className="tf-form-row">
                <div className="tf-form-group">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    required
                    className={erros.data_nascimento ? 'tf-erro-input' : ''}
                    inputMode="numeric"
                  />
                  {erros.data_nascimento && <span className="tf-msg-erro">{erros.data_nascimento}</span>}
                </div>
                <div className="tf-form-group">
                  <label>Género</label>
                  <select
                    ...
                    onChange={handleChange}
                    required
                    className={erros.genero ? 'tf-erro-input' : ''}
                  >
                  ...
                  {erros.genero && <span className="tf-msg-erro">{erros.genero}</span>}
                </div>
              </div>

              {erroGeral && (
                <p className="tf-msg-erro" role="alert" aria-live="polite">
                  {erroGeral}
                </p>
              )}
              ...
              <div className="tf-actions-row">
                <div className="tf-form-buttons">
                  <button
                    type="button"
                    className="tf-action-btn tf-action-btn--ghost"
                    onClick={() => navigate('/gestor/formadores')}
                    disabled={uploading}
                  >
                    Cancelar
                  </button>
                  <button type="submit" className="tf-action-btn tf-action-btn--primary" disabled={uploading}>
